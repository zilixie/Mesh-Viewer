...
void main() {
	...
	float radiance = min(0.2 / (r * r / 1e3), 9.0); // r2
	...
	if (render_mode == 0) {
		...
		if (useTex == 1) {
			...
			outColor = vec4(ShadePBRSpecular(kd, ks, kg/100.0, normalize(vertex_normal_3d), light_d, light_d) * radiance, 1.0);
		}
	}
